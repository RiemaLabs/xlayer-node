...
var chainIDToSha256 = map[string]string{
	"nubit":      "9727aac9bbfb021ce7fc695a92f901986421283a891b89e0af97bc9fad187793",
...
func downloadGenesisCommand() *cobra.Command {
	cmd := &cobra.Command{
		...
		Long: "Download genesis file from https://github.com/riemalabs/networks.\n" +
			"The first argument should be a known chain-id. Ex. nubit, mocha-4, or arabica-10.\n" +
			"If no argument is provided, defaults to nubit.\n",
		Args: cobra.MaximumNArgs(1),
		RunE: func(cmd *cobra.Command, args []string) error {
			chainID := getChainIDOrDefault(args)
			if !isKnownChainID(chainID) {
				return fmt.Errorf("unknown chain-id: %s. Must be: nubit, mocha-4, or arabica-10", chainID)
			}
