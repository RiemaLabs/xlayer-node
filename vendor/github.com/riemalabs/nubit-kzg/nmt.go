...
func (n *NamespacedMerkleTree) ProveNamespace(nID namespace.ID) (NamespaceRangeProof, error) {
	...
	if !found {
		...
		return n.buildAbsansProof(preIndex, postIndex)
	}
...
func (n *NamespacedMerkleTree) buildAbsansProof(preIndex, postIndex int) (NamespaceRangeProof, error) {
	...
	preProof, _, err := n.treeHasher.Open(n.leafHashes, preIndex)
	if err != nil {
		return res, err
	}
	res.preIndex = preIndex
	res.openPreIndex = KzgOpen{preIndex, n.leaves[preIndex], preProof}

	postProof, _, err := n.treeHasher.Open(n.leafHashes, postIndex)
	if err != nil {
		return res, err
	}
	res.postIndex = postIndex
	res.openPostIndex = KzgOpen{postIndex, n.leaves[postIndex], postProof}
...
func (n *NamespacedMerkleTree) buildRangeProof(proofStart, proofEnd int) (NamespaceRangeProof, error) {
	res := NamespaceRangeProof{
	...
	startProof, _, err := n.treeHasher.Open(n.leafHashes, proofStart)
	if err != nil {
		return res, err
	}
	...
	res.openStart = KzgOpen{
		index: proofStart,
		value: n.leaves[proofStart],
		proof: startProof,
	}

	proofEnd = proofEnd - 1
	endProof, _, err := n.treeHasher.Open(n.leafHashes, proofEnd)
	if err != nil {
		return res, err
	}
	res.end = proofEnd
