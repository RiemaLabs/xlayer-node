...
const (
	...
	kzgNamespaceFlagged = 0x7702

	NmtKzgSize = 2*share.NamespaceSize + kzg.CommitmentSize
...
func init() {
	...
	mhcore.Register(kzgNamespaceFlagged, func() hash.Hash {
		nh := &kzg.EmptyKzgHash{NamespaceLen: share.NamespaceSize} /*, true*/
		nh.Reset()
		return nh
	})
}
...
// CidFromNamespacedKzg uses a hash from an nmt tree to create a CID
func CidFromNamespacedKzg(namespacedHash []byte) (cid.Cid, error) {
	if got, want := len(namespacedHash), NmtKzgSize; got != want {
		return cid.Cid{}, fmt.Errorf("invalid namespaced commit length, got: %v, want: %v", got, want)
	}
...
func Translate(dah *da.DataAvailabilityHeader, row, col int) (cid.Cid, int) {
	if rand.Intn(2) == 0 { //nolint:gosec
		return MustCidFromNamespacedKzg(dah.ColumnRoots[col]), row
	}

	return MustCidFromNamespacedKzg(dah.RowRoots[row]), col
}
