...
const (
	// Bridge is a Nubit Node that bridges the Nubit consensus network and data availability
	// network. It maintains a trusted channel/connection to a Nubit Core node via the core.Client
	// API.
	Bridge Type = iota + 1
	// Light is a stripped-down Nubit Node which aims to be lightweight while preserving the highest
	// possible security guarantees.
	Light
	// Full is a Nubit Node that stores blocks in their entirety.
	Full
)
