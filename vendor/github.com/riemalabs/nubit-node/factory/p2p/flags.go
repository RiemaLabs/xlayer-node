...
const EnvCustomNetwork = "NUBIT_CUSTOM"
...
func parseNetworkFromEnv() (Network, error) {
	...
	// format: NUBIT_CUSTOM=<netID>:<genesisHash>:<bootstrapPeerList>
	if custom, ok := os.LookupEnv(EnvCustomNetwork); ok {
		fmt.Print("\n\nWARNING: Nubit custom network specified. Only use this option if the node is " +
