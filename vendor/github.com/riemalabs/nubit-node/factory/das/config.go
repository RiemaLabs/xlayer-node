...
func DefaultConfig(tp node.Type) Config {
	...
	case node.Full:
		...
		// Full node will primarily use p2p protocol for sampling, that is much more efficient and can
		// fully utilize nodes bandwidth with lower amount of parallel sampling workers
		cfg.ConcurrencyLimit = 6
		// Full node uses p2p with fallback to ipld to sample, so need 2x amount of time in worst case
		// scenario
