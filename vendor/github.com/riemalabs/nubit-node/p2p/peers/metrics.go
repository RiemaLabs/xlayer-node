...
const (
	...
	sourceKey                  = "source"
	sourceP2pSub  peerSource = "p2psub"
...
var meter = otel.Meter("p2p_peer_manager")
...
type metrics struct {
	...
	p2pPools               metric.Int64ObservableGauge // attributes: pool_status
...
func initMetrics(manager *Manager) (*metrics, error) {
	...
	p2pPools, err := meter.Int64ObservableGauge("peer_manager_pools_gauge",
	...
	callback := func(_ context.Context, observer metric.Observer) error {
		for poolStatus, count := range manager.p2pPools() {
			observer.ObserveInt64(p2pPools, count,
	...
	_, err = meter.RegisterCallback(callback, p2pPools, fullNodesPool, blacklisted)
	if err != nil {
...
func (m *metrics) validationObserver(validator p2psub.ValidatorFn) p2psub.ValidatorFn {
	if m == nil {
		return validator
	}
	return func(ctx context.Context, id peer.ID, n p2psub.Notification) pubsub.ValidationResult {
...
// p2pPools collects amount of p2p pools by poolStatus
func (m *Manager) p2pPools() map[poolStatus]int64 {
	m.lock.Lock()
	defer m.lock.Unlock()

	p2pPools := make(map[poolStatus]int64)
	for _, p := range m.pools {
		if !p.isValidatedDataHash.Load() {
			p2pPools[poolStatusCreated]++
			continue
		}

		// pool is validated but not synced
		p2pPools[poolStatusValidated]++
	}

	p2pPools[poolStatusBlacklisted] = int64(len(m.blacklistedHashes))
	return p2pPools
}
