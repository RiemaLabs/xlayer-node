...
var log = logging.Logger("p2p/peer-manager")

// Manager keeps track of peers coming from p2p.Sub and from discovery
type Manager struct {
	lock   sync.Mutex
	params Parameters
	...
	// pools collecting peers from p2pSub and stores them by datahash
	...
	// messages from p2p.Sub with height below storeFrom will be ignored, since we don't need to
...
func (m *Manager) Start(startCtx context.Context) error {
	...
	// pools will only be populated with senders of p2psub notifications if the WithP2pSubPools
	// option is used.
	if m.p2pSub == nil && m.headerSub == nil {
		return nil
	}

	validatorFn := m.metrics.validationObserver(m.Validate)
	err := m.p2pSub.AddValidator(validatorFn)
	if err != nil {
		return fmt.Errorf("registering validator: %w", err)
	}
	err = m.p2pSub.Start(startCtx)
	if err != nil {
		return fmt.Errorf("starting p2psub: %w", err)
	}
...
func (m *Manager) Stop(ctx context.Context) error {
	...
	if m.headerSub == nil && m.p2pSub == nil {
		return nil
	}
...
// Peer returns peer collected from p2p.Sub for given datahash if any available.
...
func (m *Manager) Peer(ctx context.Context, datahash share.DataHash, height uint64,
	...
	if ok {
		...
		return m.newPeer(ctx, datahash, peerID, sourceP2pSub, p.len(), 0)
	}
	...
	case peerID = <-p.next(ctx):
		...
		return m.newPeer(ctx, datahash, peerID, sourceP2pSub, p.len(), time.Since(start))
...
func (m *Manager) Validate(_ context.Context, peerID peer.ID, msg p2psub.Notification) pubsub.ValidationResult {
	...
	logger.Debugw("got hash from p2p-sub")
