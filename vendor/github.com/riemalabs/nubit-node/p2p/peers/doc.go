// Package peers provides a peer manager that handles peer discovery and peer selection for the p2p getter.
//
...
//
// The peer manager is a mechanism to store peers from p2psub, a mechanism that
// handles "peer discovery" and "peer selection" by relying on a p2psub subscription
...
//
// The peer manager is passed to the p2p getter and is used at request time to
...
//
//	peerManager := peers.NewManager(headerSub, p2pSub, discovery, host, connGater, opts...)
//
