...
import (
	...
	p2p_pb "github.com/riemalabs/nubit-node/p2p/p2peds/pb"
)

// Server is responsible for serving ODSs for blocksync over the P2P/EDS protocol.
type Server struct {
	ctx    context.Context
...
// NewServer creates a new P2P/EDS server.
func NewServer(params *Parameters, host host.Host, store *eds.Store) (*Server, error) {
	if err := params.Validate(); err != nil {
		return nil, fmt.Errorf("p2p-eds: server creation failed: %w", err)
	}
