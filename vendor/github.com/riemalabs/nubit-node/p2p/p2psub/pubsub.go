...
import (
	...
	pb "github.com/riemalabs/nubit-node/p2p/p2psub/pb"
)

var log = logging.Logger("p2p-sub")
...
func (s *PubSub) Subscribe() (*Subscription, error) {
	if s.topic == nil {
		return nil, fmt.Errorf("p2p-sub: topic is not started")
	}
...
func (s *PubSub) Broadcast(ctx context.Context, notification Notification) error {
	...
	if err != nil {
		return fmt.Errorf("p2p-sub: marshal notification, %w", err)
	}
