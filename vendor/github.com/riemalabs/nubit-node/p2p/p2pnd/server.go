...
// Server implements server side of p2p/nd protocol to serve namespaced share to remote
// peers.
type Server struct {
...
func NewServer(params *Parameters, host host.Host, store *eds.Store) (*Server, error) {
	if err := params.Validate(); err != nil {
		return nil, fmt.Errorf("p2p-nd: server creation failed: %w", err)
	}
...
func (srv *Server) sendNamespacedShares(shares share.NamespacedShares, stream network.Stream) error {
	for _, row := range shares {
		proof := row.Proof.ToPbNamespaceRangeProof()
