...
import (
	...
	blob "github.com/RiemaLabs/nubit-validator/utils/tk/sh/blob/types"
...
func Deconstruct(s Square, decoder types.TxDecoder) (core.Txs, error) {
	...
	wpfbShareRange, err := shares.GetShareRangeForNamespace(s[txShareRange.End:], namespace.SubmitBlobPaymentNamespace)
	if err != nil {
		return nil, err
	}
	...
	for i, wpfbBytes := range wpfbs {
		...
		pfb, isPfb := pfbMsgs[0].(*blob.MsgSubmitBlobPayments)
		if !isPfb {
...
func (s Square) WrappedPFBs() (core.Txs, error) {
	wpfbShareRange, err := shares.GetShareRangeForNamespace(s, namespace.SubmitBlobPaymentNamespace)
	if err != nil {
		return core.Txs{}, nil
	}
