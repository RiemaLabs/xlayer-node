...
type Tree interface {
	...
	ProveRange(start, end int) (*kzg.NamespaceRangeProof, error)
}
...
func NewErasuredNamespacedMerkleTree(squareSize uint64, axisIndex uint, options ...kzg.Option) ErasuredNamespacedMerkleTree {
	if squareSize == 0 {
		panic("cannot create a ErasuredNamespacedMerkleTree of squareSize == 0")
	}
	options = append(options, kzg.NamespaceIDSize(appconsts.NamespaceSize))
	//options = append(options, kzg.IgnoreMaxNamespace(true))
	tree := kzg.New(appconsts.NewBaseHashFunc(), options...)
...
func NewConstructor(squareSize uint64, opts ...kzg.Option) rsmt2d.TreeConstructorFn {
	return constructor{
...
func (w *ErasuredNamespacedMerkleTree) ProveRange(start, end int) (*kzg.NamespaceRangeProof, error) {
