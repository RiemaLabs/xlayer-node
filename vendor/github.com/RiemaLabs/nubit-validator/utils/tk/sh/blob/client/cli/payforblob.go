...
const (
	...
	// submitting a SubmitBlobPayment.
...
func CmdSubmitBlobPayment() *cobra.Command {
	cmd := &cobra.Command{
		Use: "SubmitBlobPayments namespaceID blob",
		// This example command can be run in a new terminal after running single-node.sh
		Example: "nubit-validatord tx blob SubmitBlobPayments 0x00010203040506070809 0x48656c6c6f2c20576f726c6421 \\\n" +
			"\t--chain-id private \\\n" +
			"\t--from validator \\\n" +
			"\t--keyring-backend test \\\n" +
			"\t--fees 21000unub \\\n" +
			"\t--yes",
		Short: "Pay for a data blob to be published to Nubit.",
		Long: "Pay for a data blob to be published to Nubit.\n" +
		...
		Aliases: []string{"SubmitBlobPayment"},
		Args: func(cmd *cobra.Command, args []string) error {
			if len(args) < 2 {
				return fmt.Errorf("SubmitBlobPayments requires two arguments: namespaceID and blob")
			}
			return nil
		},
...
// this private func is used in CmdSubmitBlobPayment and CmdTestRandBlob
func broadcastPFB(cmd *cobra.Command, blob *types.Blob) error {
	...
	// See https://github.com/RiemaLabs/nubit-validator/issues/1041
	pfbMsg, err := types.NewMsgSubmitBlobPayments(clientCtx.FromAddress.String(), blob)
	if err != nil {
		return err
	}
