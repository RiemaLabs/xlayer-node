...
// returns an error if tx contains a MsgSubmitBlobPayments where the total blob size is
...
func (d MaxTotalBlobSizeDecorator) AnteHandle(ctx sdk.Context, tx sdk.Tx, simulate bool, next sdk.AnteHandler) (sdk.Context, error) {
	...
	for _, m := range tx.GetMsgs() {
		if pfb, ok := m.(*blobtypes.MsgSubmitBlobPayments); ok {
...
func (d MaxTotalBlobSizeDecorator) getMaxSquareSize(ctx sdk.Context) int {
	...
	// nubit-validator does not currently use those. see this PR for more details
