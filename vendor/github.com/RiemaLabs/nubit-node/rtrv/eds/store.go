...
func (s *Store) getCAR(ctx context.Context, root share.DataHash) (io.ReadCloser, error) {
	...
	// accessor in the cache here because getCAR is used by p2p-eds. There is a lower probability,
...
// registered on the Store. NOTE: The blockstore does not store whole Nubit Blocks but IPFS
...
// store whole Nubit Blocks but IPFS blocks. We represent `shares` and NMT Merkle proofs as IPFS
...
func dahFromCARHeader(carHeader *carv1.CarHeader) *share.Root {
	...
	for _, root := range carHeader.Roots {
		rootBytes = append(rootBytes, ipld.NamespacedKzgFromCID(root))
	}
	return &share.Root{
...
// func hashRootsFromCARHeader(carHeader *carv1.CarHeader) *share.HashRoot {
// 	rootCount := len(carHeader.Roots)
// 	rootBytes := make([][]byte, 0, rootCount)
// 	for _, root := range carHeader.Roots {
// 		rootBytes = append(rootBytes, ipld.NamespacedSha256FromCID(root))
// 	}
// 	return &share.HashRoot{
// 		RowRoots:    rootBytes[:rootCount/2],
// 		ColumnRoots: rootBytes[rootCount/2:],
// 	}
// }
