...
func (a *ProofsAdder) VisitFn() kzg.NodeVisitorFn {
	if a == nil {
		return nil
	}
...
func (a *ProofsAdder) visitInnerNodes(hash []byte, children ...[]byte) {
	switch len(hash) {
	case NkzgCommitmentSize:
		id := MustCidFromKzg(hash[2*share.NamespaceSize:])
		size := len(children)
		rowData := make([]byte, 0, size*NmtHashSize)
		for _, child := range children {
			if len(child) != NmtHashSize {
				panic("unexpected Leaf hash size")
			}
			rowData = append(rowData, child...)
		}
		a.addProof(id, rowData)
	default:
		panic("expected nmt hash")
	}
}
