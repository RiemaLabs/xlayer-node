...
type Module interface {
	...
	// Balance retrieves the Nubit coin balance for the node's account/signer
	// and verifies it against the corresponding block's AppHash.
	Balance(ctx context.Context) (*state.Balance, error)
	// BalanceForAddress retrieves the Nubit coin balance for the given address and verifies
	...
	// Nubit network and blocks until the tx is included in
	// a block.
...
func (api *API) SubmitBlob(
	ctx context.Context,
	fee state.Int,
	gasLim uint64,
	blobs []*blob.Blob,
) (*state.TxResponse, error) {
	return api.Internal.SubmitBlob(ctx, fee, gasLim, blobs)
}
