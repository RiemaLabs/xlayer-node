...
import (
	...
	"github.com/RiemaLabs/nubit-validator/tx"
	apperrors "github.com/RiemaLabs/nubit-validator/auxiliary/errors"
	"github.com/RiemaLabs/nubit-validator/utils/appconsts"
	appblob "github.com/RiemaLabs/nubit-validator/utils/tk/sh/blob"
...
type CoreAccessor struct {
	...
	lock            sync.Mutex
	lastSubmitBlobPayment  int64
	blobPaymentCount int64
...
// NewCoreAccessor dials the given nubit-core endpoint and
...
func (ca *CoreAccessor) Start(ctx context.Context) error {
	...
	// dial given nubit-core endpoint
...
// SubmitBlob builds, signs, and synchronously submits a MsgSubmitBlobPayment. It blocks until the
...
func (ca *CoreAccessor) SubmitBlob(
	ctx context.Context,
	fee Int,
	gasLim uint64,
	blobs []*blob.Blob,
	...
	for attempt := 0; attempt < maxRetries; attempt++ {
		response, err := appblob.SubmitBlob(
			ctx,
			ca.signer,
			ca.coreConn,
...
func (ca *CoreAccessor) LastSubmitBlobPayment() int64 {
	ca.lock.Lock()
	defer ca.lock.Unlock()
	return ca.lastSubmitBlobPayment
}

func (ca *CoreAccessor) SubmitBlobPaymentCount() int64 {
	ca.lock.Lock()
	defer ca.lock.Unlock()
	return ca.blobPaymentCount
}

func (ca *CoreAccessor) markSuccessfulPFB() {
	ca.lock.Lock()
	defer ca.lock.Unlock()
	ca.lastSubmitBlobPayment = time.Now().UnixMilli()
	ca.blobPaymentCount++
}
