...
// KzgCommitment is a Merkle Root of the subtree built from shares of the Blob.
// It is computed by splitting the blob into shares and building the Merkle subtree to be included
// after Submit.
type KzgCommitment []byte

func (com KzgCommitment) String() string {
	return string(com)
}

// Equal ensures that commitments are the same
func (com KzgCommitment) Equal(c KzgCommitment) bool {
	return bytes.Equal(com, c)
}

// Proof is a collection of kzg.Proofs that verifies the inclusion of the data.
...
// https://github.com/RiemaLabs/nubit-node/issues/2303
type Proof []*kzg.NamespaceRangeProof
...
// Blob represents any application-specific binary data that anyone can submit to Nubit.
type Blob struct {
	types.Blob `json:"blob"`

	Commitment KzgCommitment `json:"commitment"`

	// the nubit-node's namespace type
...
func (b *Blob) compareCommitments(com KzgCommitment) bool {
