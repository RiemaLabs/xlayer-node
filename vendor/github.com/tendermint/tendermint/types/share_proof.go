...
func (sp ShareProof) VerifyProof() bool {
	...
	for i, proof := range sp.ShareProofs {
		nmtProof := kzg.NewInclusionProof(
			int(proof.Start),
			int(proof.End),
			int(proof.PreIndex),
			int(proof.PostIndex),
			ToNmtKzgOpen(proof.OpenStart),
			ToNmtKzgOpen(proof.OpenEnd),
			ToNmtKzgOpen(proof.OpenPreIndex),
			ToNmtKzgOpen(proof.OpenPostIndex),
		)
...
func ToNmtKzgOpen(p *types.KzgOpen) kzg.KzgOpen {
	return kzg.NewKzgOpen(int(p.Index), p.Value, p.Proof)

}

func FromNmtKzgOpen(p kzg.KzgOpen) *types.KzgOpen {
	return &types.KzgOpen{
		Index: int32(p.Index()),
		Value: p.Value(),
		Proof: p.Proof(),
	}
